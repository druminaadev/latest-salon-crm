...
export default function MembershipPage() {
  ...
  const openEdit = (plan: typeof initialPlans[number]) => {
    setEditId(plan.id)
    setForm({ name: plan.name, price: String(plan.price), duration: plan.duration, services: String(plan.services), discount: String(plan.discount) })
    setShowModal(true)
  }
  ...
  const handleSave = () => {
    ...
    if (editId !== null) {
      setPlans(p => p.map(x => x.id === editId ? { ...x, name: form.name, price: Number(form.price), duration: form.duration || '1 Month', services: Number(form.services) || 0, discount: Number(form.discount) || 0 } : x))
    } else {
          ...
          <div key={plan.id} className="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 p-6 hover:shadow-md transition group">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                <button className="p-1.5 rounded-lg text-gray-400 hover:text-salon-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition" onClick={() => openEdit(plan)}><Edit2 size={13} /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl p-6 shadow-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-gray-900 dark:text-white">{editId !== null ? 'Edit Plan' : 'Add Plan'}</h2>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={handleSave} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-salon-600 hover:bg-salon-900 text-white text-sm font-semibold transition"><Save size={14} /> {editId !== null ? 'Update' : 'Save'}</button>
            </div>
          </div>
        </div>
      )}
